# -*- coding: utf-8 -*-
"""
Created on Tue Oct 14 16:13:39 2025
...
def discount_factors_ramsey_relative(N, n,base_year_index,rho, eta, growth_consumption):
    ...
    DF[base_year_index] = 1.0  #the DF in the base year is set to 1
...
#scc
#scc relative to the first year 1950
#scc=np.sum(delta_D[n:]*discount_ramsey[:])

#scc=np.sum(delta_D[275:]*discount_ramsey[75:])

scc=np.sum(delta_D[n:]*discount_ramsey[:])

#there is no GDP growth on the first year with GDP
#scc discount to present value
#scc_pv=np.sum(delta_D[n+1:]*discount_ramsey_relative[1:])

#calculate the scc in 2025
#scc_pv_2025=np.sum(delta_D[275:]*discount_ramsey_relative[75:])

scc_pv_2025=np.sum(delta_D[n+1:]*discount_ramsey_relative[1:])
